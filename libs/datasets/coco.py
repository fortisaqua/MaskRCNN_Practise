...
def read(tfrecords_filename):
  ...
  gt_boxes_ = tf.reshape(gt_boxes, [num_instances, 5])
  gt_masks = tf.decode_raw(features['label/gt_masks'], tf.uint8)
  gt_masks_ = tf.cast(gt_masks, tf.int32)
  gt_masks__ = tf.reshape(gt_masks_, [num_instances, ih, iw])


  return image, ih, iw, gt_boxes_, gt_masks__, num_instances, img_id
